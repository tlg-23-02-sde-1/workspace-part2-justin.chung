...
public class InMemoryCatalog implements Catalog {
    ...
    private List<MusicItem> catalogData = new ArrayList<>(List.of(
        ...
        new MusicItem(8L,  "So Much for the Afterglow", "Everclear",                 "1997-01-19", 13.99, ROCK),
        new MusicItem(9L,  "Surfacing",                 "Sarah McLachlan",           "1997-12-04", 17.97, MusicCategory.ALTERNATIVE),
        new MusicItem(10L, "Hysteria",                  "Def Leppard",               "1987-06-20", 17.97, ROCK),
        new MusicItem(11L, "A Life of Saturdays",       "Dexter Freebish",           "2000-12-06", 16.97, MusicCategory.RAP),
        new MusicItem(12L, "Human Clay",                "Creed",                     "1999-10-21", 18.97, ROCK),
        ...
        new MusicItem(15L, "Big Ones",                  "Aerosmith",                 "1994-05-08", 18.97, ROCK),
        new MusicItem(16L, "90125",                     "Yes",                       "1983-10-16", 11.97, ROCK),
        new MusicItem(17L, "1984",                      "Van Halen",                 "1984-08-19", 11.97, ROCK),
     ...
     */
    public Collection<MusicItem> titleSameAsArtist() {
        Collection<MusicItem> result = new ArrayList<>();
        for(MusicItem item: catalogData) {
    ...
    public Collection<MusicItem> rockItemPrice(double price) {
        Collection<MusicItem> result = new ArrayList<>();
        for(MusicItem item: catalogData) {
            if(item.getMusicCategory().equals(ROCK) && (item.getPrice() <= price)) {
                result.add(item);
            }
        }
        return result;
     ...
     */
    public MusicItem cheapestItemInGenre(MusicCategory category) {
        MusicItem result = null;
        for(MusicItem item:catalogData) {
            if(item.getMusicCategory().equals(category)) {
                if(result == null) {
                    result = item;
                } else if(item.getPrice() < result.getPrice()) {
                    result = item;
                }
            }
        }
     ...
     */
    public double averagePriceOfGenre(MusicCategory category) {
        double result = 0.0;
        int counter = 0;
        for(MusicItem item:catalogData) {
            if(item.getMusicCategory().equals(category)) {
     ...
     * Just the titles!
     */
    public Set<String> sortedPopTitles() {
        Set<String> result = new TreeSet<>();
        for(MusicItem item:catalogData) {
            if(item.getMusicCategory() == POP) {
     ...
     */
    public Collection<MusicItem> releasedIn80s(double price) {
        Collection<MusicItem> result = new ArrayList<>();
        for(MusicItem item:catalogData) {
            if(item.getReleaseDate().after(new Date(79,12,31))
            && item.getReleaseDate().before(new Date(90,1,1))){
